...
class AdminTelegramBot:
	...
	def start_telegram_bot(self, context: telegram.ext.callbackcontext.CallbackContext, chat_id: int, message_id: int, telegram_bot_id: int) -> None: # Метод для кнопки 1:1
		...
		if telegram_bot_name not in telegram_bots:
			result = GlobalFunctions.start_telegram_bot(telegram_bot_name=telegram_bot_name)
	...
	def telegram_bot_settings(self, context: telegram.ext.callbackcontext.CallbackContext, chat_id: int, message_id: int, telegram_bot_id: int) -> None: # Метод для кнопки 1:3
		...
		message = f'<b>Настройки {telegram_bot_name} Telegram бота:</b>'
		message += f"\n1. Тип {telegram_bot_name} Telegram бота: <i>{'Приватный' if self.config[f'{telegram_bot_name}TelegramBot']['Private'] == '1' else 'Не приватный'}</i>"
		message += f"\n2. Токен {telegram_bot_name} Telegram бота: <i>{self.config[f'{telegram_bot_name}TelegramBot']['Token']}</i>"
	...
	def telegram_bot_users(self, context: telegram.ext.callbackcontext.CallbackContext, chat_id: int, message_id: int, telegram_bot_id: int) -> None: # Метод для кнопки 1:4
		...
		message, num = f'<b>Список пользователей {telegram_bot_name} Telegram бота:</b>', 1
		for user in users:
		...
		if message == f'<b>Список пользователей {telegram_bot_name} Telegram бота:</b>':
